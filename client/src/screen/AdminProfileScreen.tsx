...
//       const res = await axios.post('http://192.168.0.111:5001/admindata', { token });
...
//     try {
//       const res = await axios.post('http://192.168.0.111:5001/get-admin-stats', { adminId });
//       if (res.data.status === 'ok') {
...
//     try {
//       const token = await AsyncStorage.getItem('token');
//       const res = await axios.post('http://192.168.0.111:5001/update-profile-pic', {
//         token,
...
const AdminProfileScreen = () => {
  ...
  if (result.didCancel || !result.assets || result.assets.length === 0) {
    console.log('Image pick cancelled or no assets');
    return;
  }

  const imageAsset = result.assets[0];
  if (!imageAsset.base64) {
    Alert.alert('Error', 'Image conversion failed. Please try a different image.');
    return;
  }

  const mimeType = imageAsset.type || 'image/jpeg';
  const base64Image = `data:${mimeType};base64,${imageAsset.base64}`;
