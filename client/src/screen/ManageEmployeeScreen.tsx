...
type Employee = {
  ...
  name: string;
  department: string;
  post: string; // changed from role to post
  email: string;
  image?: any;
  profilePic?: string;
};
...
const BACKEND_URL = BASE_URL; // change this to your server address
...
const formattedApproved = approvedRes.data.data.map((user: any) => ({
  ...
  try {
    ...
    const res = await axios.post(`${BACKEND_URL}/handle-request`, {
      employeeId,
      action,
      adminId, // ✅ this enables backend to count handled approvals
    });
